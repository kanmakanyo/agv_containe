...
bool autonomous_flag;  //Update 291221 backup pendant
/*--------------------------------------------------------------------*/
...
void receive_message(const geometry_msgs::PoseWithCovarianceStamped& sub_msg) {
  //  autonomous_flag = sub_msg.pose.covariance[3];/
  if (!pendantControl) {
    steer_front_setpoint_rad = min(max(sub_msg.pose.covariance[0], STEER_FRONT_MIN_RAD), STEER_FRONT_MAX_RAD);
    throttle_setpoint = min(max(sub_msg.pose.covariance[1], THROTTLE_MIN), 2);//THROTTLE_MAX);
...
bool isPendantMode() {
  if (!digitalRead(INPUT_PENDANT_MODE_PIN)) {
    if (millis() - timer_pendant > 200) {
      return true; //awalnya 200
    }
    else {
      return false;
    }
...
void steer_vD2() {
  ...
  if (abs(steer_front_setpoint_bit - steer_front_actual_bit) > 5) { // threshold
    if (steer_front_setpoint_rad > steer_front_actual_rad) {
      digitalWrite(ST0_SIG, LOW); // Move to left
      digitalWrite(ST1_SIG, HIGH);
      ...
      digitalWrite(ST0_SIG, HIGH);
      digitalWrite(ST1_SIG, LOW); // Move to right
...
void MSRB(String condition) //setting mode selector and regenerative brake
{
  if (condition == "normal") {
    ...
    digitalWrite(RWL_BRAKE, HIGH);
    delay(3500); //wait for approximately 3 seconds, then back to normal
    ...
    //    digitalWrite(FWL_BRAKE, HIGH);
    delay(3500); //keep it for approximately 3 seconds, then back to normal
    //Serial.println("Ready!");
    MSRB("normal");
  }
}
...
void setup() {
  ...
  //  pendantControl = !autonomous_flag; //isPendantMode();/
...
/*----------------------------- MAIN LOOP ----------------------------*/
void loop() {
  pendantControl = isPendantMode();//!autonomous_flag;
  propulsion_vD1();
  steer_vD2();
  ...
  if ((millis() - ros_time) >= ros_period) {
    ...
    pub_msg.pose.covariance[13] = autonomous_flag;//locker_setpoint;
