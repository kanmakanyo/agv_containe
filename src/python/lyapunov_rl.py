...
class Controller(object):
    def __init__(self, kp, ki, kd, feed_forward_params, sat_long,\
        ...
        # RL Agent
        self.agent = Agent()
        agent_param = "ckpt_3000_ppo_anet_params.pkl"
        agent_param_path = os.path.abspath(sys.path[0] + '/../src/python/' + agent_param)
    ...
    def _update_error(self, x, y, v, yaw):
        ...
        v_ref = self._waypoints[self._closest_idx, 3]
        if v_ref <= 0.1:
            None
        else:
            v_ref = v_ref / (1.0 + self._kv_lat*self._e_lat**2 + self._kv_yaw*self._e_yaw**2)
            v_ref = np.fmax(v_ref, self._min_vel_move)
        self._ev = v_ref - v
        ...
        if self.wp_loop == 0 : #TODO: looping waypoint (wp_loop > 0)
            self.bounded_wp_s_idx = self._idx_wp - int(self.bounded_wp_dist_rear/self.inter_wp_min_dist)
            self.bounded_wp_s_idx = max(self.bounded_wp_s_idx, 0) #0 if self.bounded_wp_s_idx < 0 else self.bounded_wp_s_idx
            self.bounded_wp_e_idx = self._idx_wp + int(self.bounded_wp_dist_front/self.inter_wp_min_dist)
            self.bounded_wp_e_idx = min(self.bounded_wp_e_idx, len(self._waypoints)-1) #(len(self._waypoints)-1) if self.bounded_wp_e_idx > (len(self._waypoints)-1) else self.bounded_wp_e_idx
        self.loc_waypoints = self._waypoints[self.bounded_wp_s_idx:self.bounded_wp_e_idx]
        ####
        ...
        self._vref = v_ref #self._waypoints[self._closest_idx, 3]
    ...
    def calculate_control_signal(self, dt, x, y, v, yaw):
        ...
        ### Sum
        d = np.arctan(a + b + c)
        # d = d + self._feed_forward_lateral()
